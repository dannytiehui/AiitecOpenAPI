 ...
 *
 * 内置规则(仅对继承`AIIIqPacket`的子类)：
 *
 * 1. 关于属性query的值：<br />
 *    当前类名称加上Query后缀的对象。如:`AIIDetailsRequest`类,则其query属性值默认为`AIIDetailsRequestQuery`的实例化对象.
 * 2. 关于属性queryClass的值：<br />
 *    当前类名称加上Query后缀后的类。如:`AIIDetailsRequest`类,则其queryClass属性值默认为`[AIIDetailsRequestQuery class]`.
 * 3. 关于属性nameSpace的值：<br />
 *    去掉AII前缀,去掉Request/Response后缀的字符串。如:`AIIUserDetailsRequest`/`AIIUserDetailsResponse`类,则其nameSpace属性值默认为UserDetails.
 *
 */
